...
def main():
    ...
    for i, slot in enumerate(BODY_ORDER):
        ...
        # Xsens Acc_X/Y/Z behaves as sensor-local gravity-included acceleration.
        # Convert to world, then body frame, then use T-pose mean as offset.
        R_seq = R_raw_all[slot][tp0:tp1]
        a_sensor_seq = A_sensor_all[slot][tp0:tp1]
        a_world_seq = np.einsum("tij,tj->ti", R_seq, a_sensor_seq)
        a_body_seq = (smpl2imu @ a_world_seq.T).T
    ...
    for i, slot in enumerate(BODY_ORDER):
        ...
        a_sensor_seq = A_sensor_all[slot][f0:f1]
